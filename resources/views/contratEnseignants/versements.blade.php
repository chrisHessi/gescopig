...
@section('content')
    ...
    <div class="content">
        {{Form::open([route('contratEnseignants.save', [$contrat->id])])}}
        <div class="box box-primary">  
            <div class="box-body">
                ...
                <div class="form-group col-xs-3 doc">
                    ...
                    {!! Form::select('tranche', ['4/5'=>'4/5', '1/5' => '1/5', '100%' => 'Totalité'], null, ['class' => 'form-control']) !!}
                </div>
                ...
                <table class="table table bordered">
                    ...
                    <tbody>
                        @foreach($ecues as $ecue)
                            <tr>
                                <td>{{ Form::radio('ecue_id', $ecue->id) }}</td>
                                <td>{{ $ecue->title }}</td>
                                <td>{{ $ecue->semestre->title.' '.$ecue->semestre->niveau }}</td>
                                <td id="{{ 'th-'.$ecue->id }}">{{ ($ecue->semestre->cycle->label == 'Licence') ? $contrat->mh_licence : $contrat->mh_master }}</td>
                                <td>
                                    @foreach($ecue->enseignements->where('contrat_enseignant_id', $contrat->id) as $enseignement)
                                        <li class="list-group-item">
                                            <label class="checkbox-inline">
                                                <label class="checkbox-inline">
                                                    {!! Form::checkbox('specialite[]', $enseignement->specialite->id,null,['id' => $enseignement->specialite->id, 'class' => 'cases-'.$ecue->id]) !!}
                                                    {!! Form::label($enseignement->specialite->id, $enseignement->specialite->slug)!!}
                                                </label>
                                            </label>
                                        </li>
                                    @endforeach
